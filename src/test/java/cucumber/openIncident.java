...
public class openIncident {
	...
	public void he_modifies_the_incidence() throws Throwable {
		incidentsService.changeIncidentState(incident.getId(), IncidentStates.OPEN.toString());
	}

	@Then("^the incident is opened$")
	public void the_incidence_is_opened() throws Throwable {
		if (!incident.getState().equals(IncidentStates.OPEN)) {
			throw new Throwable(
					"The state of the incident " + incident.getId().toString() + " was not changed to OPEN");
		}
	}
}
