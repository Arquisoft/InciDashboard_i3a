...
public class cancelIncident {
	...
	public void he_modifies_the_incidence() throws Throwable {
		incidentsService.changeIncidentState(incident.getId(), IncidentStates.CANCELLED.toString());
	}

	@Then("^the incident is cancelled$")
	public void the_incidence_is_cancelled() throws Throwable {
		if (!incident.getState().equals(IncidentStates.CANCELLED)) {
			throw new Throwable(
					"The state of the incident " + incident.getId().toString() + " was not changed to CANCELLED");
		}
	}
}
